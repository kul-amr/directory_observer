...
Assumptions: 
	...
	2. These files will have users data - name and email which we need to parse.
	...
	5. To check if data is getting added , we have created 2 web pages - users and logs.
		users - will display total number of users and top 100 users order by craeted_date desc
		logs - will display total number of parsed files entry and all related details e.g. filename, success/error message etc.
	6. As soon as files are created, those will be added to queue and would be assigned to threads (we are currently using 5 threads)
	
Flow:	

1. run.py	:	It will start the web app and will create sqlite database named harness.db and the tables
2. harness_monitor.py	:	This is the script which will monitor the folder for new files. This will prompt for directory to monitor. e.g. path for monitor folder from current structure
...
4. create_data.py :	can create csv files for load testing.	
					This will prompt for inputs :	
												directory to create files e.g. monitor directory which is getting monitored for new file addition
...
5. To monitor the data creation in database, you can use users and logs webpage with date range as input.
	e.g	http://localhost:5000/logs?start_date=2017-09-17&end_date=2017-09-19 
